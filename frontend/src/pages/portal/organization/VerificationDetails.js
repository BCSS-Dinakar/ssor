  ...
  if (!selectedRequest) {
    return (
      <div className="space-y-6">
        <Link to="/portal/requests" className="inline-flex items-center gap-2 text-base font-semibold text-slate-500 hover:text-primary transition-colors bg-white px-4 py-2 rounded-xl border border-slate-200">
          <ArrowLeft className="h-4 w-4" /> Back to Applications List
        </Link>
        <div className="card p-12 text-center text-slate-400">
          <ShieldAlert className="h-10 w-10 text-red-500 mx-auto mb-2" />
          <h3 className="text-base font-bold text-slate-700">Reference {id} Not Found</h3>
        </div>
      </div>
    );
  }
          ...
          <div className="pt-4 border-t border-slate-100 grid sm:grid-cols-2 gap-4">
            {selectedRequest.candidateImage && (
              <div className="space-y-2">
                ...
                <div className="w-24 h-24 rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                  ...
                  <img
                    src={selectedRequest.candidateImage}
                    alt="Candidate"
            ...
            {selectedRequest.consentFile && (
              <div className="space-y-2">
                ...
                <a
                  href={selectedRequest.consentFile}
                  target="_blank"
                  rel="noreferrer"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 space-y-4">
            ...
            <div className="relative pl-7 space-y-5">
              ...
              {selectedRequest.steps.map((step, idx) => (
                <div key={idx} className="relative flex gap-3 text-sm leading-relaxed font-semibold">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 space-y-4">
            ...
            {selectedRequest.status === 'cleared' && (
              <div className="space-y-4">
                ...
                <div className="border-4 border-double border-slate-200 bg-slate-50/50 rounded-2xl p-5 space-y-4 relative overflow-hidden text-center shadow-inner">
                  ...
                  <p className="text-sm text-slate-600 leading-normal max-w-sm mx-auto font-medium">
            ...
            {selectedRequest.status === 'rejected' && (
              <div className="space-y-4">
                <div className="p-4 bg-red-50 border border-red-200 text-sm font-semibold text-red-800 rounded-2xl">
                ...
                {selectedRequest.matchedSuspect && (
                  <div className="border border-red-200 bg-red-50/30 rounded-2xl overflow-hidden mt-4">
                    ...
                    <div className="p-4 grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="block text-xs font-bold text-slate-500 uppercase">Match ID</span>
                        <span className="font-mono font-semibold text-slate-800">{selectedRequest.matchedSuspect.id}</span>
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-slate-500 uppercase">Registered Name</span>
                        <span className="font-bold text-slate-800">{selectedRequest.matchedSuspect.name}</span>
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-slate-500 uppercase">Convicted Offence</span>
                        <span className="font-semibold text-red-700">{selectedRequest.matchedSuspect.offence}</span>
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-slate-500 uppercase">Conviction Date</span>
                        <span className="font-mono font-semibold text-slate-800">{selectedRequest.matchedSuspect.convDate}</span>
                      </div>
                      <div className="col-span-2">
                        <span className="block text-xs font-bold text-slate-500 uppercase">Physical Markings</span>
                ...
                <Link 
                  ...
                  className="btn-primary py-2.5 px-5 text-sm w-full justify-center bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 shadow-red-100 mt-2"
                >
