    ...
    <div className="space-y-6 w-full animate-fadeIn pb-10">
      ...
      <div className="card p-4 flex flex-col sm:flex-row gap-3 items-center bg-white shadow-sm">
        <div className="relative flex-grow w-full">
          ...
          <input
            className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-base focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/15 transition-all placeholder-slate-400"
        ...
        {filteredCandidates.map((c) => {
          ...
          return (
            <div key={c.id} className={`card p-5 border flex flex-col justify-between hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300 ${borderClass}`}>
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-100 to-slate-200/50 flex items-center justify-center shrink-0 border border-slate-200/40 relative">
                    <span className="text-sm font-black text-slate-500 font-heading">{(c.candidateName || 'C').charAt(0)}</span>
                    <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white ${glowClass}`} />
                  </div>
                  <div className="flex-1 space-y-0.5">
                    <div className="flex items-center justify-between">
                      <h3 className="font-bold text-slate-800 text-base truncate pr-2" title={c.candidateName}>
                        {c.candidateName}
                      </h3>
                    </div>
                    <p className="text-sm font-semibold text-slate-500 truncate" title={c.role}>
                      {c.role}
                    </p>
                    <p className="text-sm text-slate-400 font-mono mt-1 flex items-center gap-1">
              ...
              <div className="mt-6 pt-3 border-t border-slate-100 flex items-center justify-end text-sm">
                <button
                  onClick={() => navigate(`/portal/candidates/${c.id}`)}
                  className="text-sm font-black text-secondary bg-blue-50 hover:bg-blue-100 px-3.5 py-2 rounded-xl transition-all duration-200 flex items-center gap-1 border border-blue-200"
                >
                  View Profile
                </button>
              </div>
      ...
      {filteredCandidates.length === 0 && (
        <div className="card p-12 text-center text-slate-400 max-w-md mx-auto bg-white">
          ...
          <span className="text-sm font-bold text-slate-500">No personnel match your search filter.</span>
        </div>
      )}
    </div>
  );
