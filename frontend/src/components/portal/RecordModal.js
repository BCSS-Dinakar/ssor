...
function Detail({ label, value, mono }) {
  return (
    <div>
      ...
      <div className={`text-sm text-primary font-medium ${mono ? 'font-mono' : ''}`}>{value}</div>
    </div>
  );
}
      ...
      <div className="bg-white rounded-none max-w-2xl w-full max-h-[88vh] overflow-auto shadow-2xl border border-slate-200">
        <div className="flex items-start justify-between p-6 border-b border-gray-100 sticky top-0 bg-white">
          <div>
            ...
            <div className="text-[10px] font-bold font-mono text-slate-400 mt-1 uppercase">{record.id} · {record.cc} · {record.ps}</div>
          </div>
        ...
        <div className="p-6">
          <div className="grid sm:grid-cols-2 gap-5 mb-6">
            ...
            <Detail label="Legal Status" value={
              <span className={`inline-flex items-center px-2 py-0.5 rounded-none border text-[9px] font-bold uppercase tracking-wider ${record.legalStatus === 'Convicted' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-none p-4 mb-5">
            <div className="text-[10px] uppercase tracking-wider text-slate-500 font-bold mb-2">Statutory basis</div>
            <div className="flex flex-wrap gap-2">
              {tier.sections.map((s) => (
                <span key={s} className="font-mono text-[9px] font-bold text-secondary bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-none uppercase">{s}</span>
              ))}
            </div>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-none p-4 mb-5 mt-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-xs font-bold font-heading text-slate-800 uppercase tracking-wider">Linked Offenders / Network</h4>
            </div>
            {record.legalStatus === 'Convicted' ? (
              <div className="text-sm text-slate-600 italic">No identified accomplices in this case.</div>
            ) : (
              <div className="space-y-2">
                <div className="flex items-center justify-between bg-white p-2.5 border border-slate-200 rounded-none text-sm">
                  <span className="font-semibold text-primary">T. Suresh Kumar (Co-accused)</span>
                  <span className="text-[10px] font-mono font-bold text-slate-400">SOR-2026-0922</span>
                </div>
                <div className="flex items-center justify-between bg-white p-2.5 border border-slate-200 rounded-none text-sm">
                  <span className="font-semibold text-primary">IP: 182.44.X.X (Cyber Link)</span>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-amber-600 bg-amber-50 px-2 py-0.5 border border-amber-200">Pending</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex justify-end gap-3 p-4 border-t border-gray-100 sticky bottom-0 bg-white">
          <button
            ...
            className="text-xs font-bold px-4 py-2.5 rounded-none border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-700 transition-colors uppercase tracking-wider"
          >
            Raise disclosure
          </button>
          <button onClick={onClose} className="btn-secondary py-2.5 px-5 text-xs font-bold rounded-none uppercase tracking-wider border-slate-300">Close</button>
          {onFlag && (
            <button onClick={() => onFlag(record)} className="btn-primary py-2.5 px-5 text-xs font-bold rounded-none uppercase tracking-wider">Flag for review</button>
          )}
        </div>
      </div>
    </div>
